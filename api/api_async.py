...
from middlewares import auth_middleware, set_request_id_middleware
...
app.register_middleware(set_request_id_middleware, 'request')
...
async def on_startup(app: Sanic, loop: asyncio.AbstractEventLoop):
    ...
    app.producer = AIOKafkaProducer(bootstrap_servers=KAFKA_BOOTSTRAP_SERVERS,
                                    loop=loop,
    ...
    app.mongo = motor.motor_asyncio.AsyncIOMotorClient(MONGO_CONNECT_URI)[MONGO_DB_NAME]
