...
async def create_mark(request: Request, user_id: str, movie_id: str, mark: int):
    doc = await request.app.ctx.mongo.movieMarks.find_one({'user_id': user_id, 'movie_id': movie_id})
    if doc is not None:
        return text('Bad request', 403)

    await request.app.ctx.mongo.movieMarks.insert_one({
        'user_id': user_id,
        'movie_id': movie_id,
        'mark': mark,
...
async def edit_mark(request: Request, user_id: str, mark_id: str, mark: int):
    old_document = await request.app.ctx.mongo.movieMarks.find_one({'user_id': user_id, '_id': ObjectId(mark_id)})
    if not old_document:
        return text('Bad request', 403)

    await request.app.ctx.mongo.movieMarks.update_one({'_id': ObjectId(mark_id)}, {'$set': {'mark': mark}})
    return text('Accepted', 202)
...
async def delete_mark(request: Request, user_id: str, mark_id: str):
    await request.app.ctx.mongo.movieMarks.find_one({'user_id': user_id, '_id': ObjectId(mark_id)})
    return text('Accepted', 202)


@movie_marks_bp.route('/movies/<movie_id:string>/likes', methods=['GET'])
async def movie_likes(request: Request, movie_id: str):
    res = await request.app.ctx.mongo.movieMarks.count_documents({'movie_id': movie_id, 'mark': {'$eq': 10}})
...
async def movie_dislikes(request: Request, movie_id: str):
    res = await request.app.ctx.mongo.movieMarks.count_documents({'movie_id': movie_id, 'mark': {'$eq': 0}})
    return json({'data': res}, 200)
...
async def movie_rating(request: Request, movie_id: str):
    ...
    async for doc in request.app.ctx.mongo.movieMarks.find({'movie_id': movie_id}):
        mark = doc['mark']
        marks.append(mark)
