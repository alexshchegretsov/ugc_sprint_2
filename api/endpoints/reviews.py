...
async def create_review(request: Request, user_id: str, movie_id: str):
    data = request.json
    await request.app.ctx.mongo.movieMarks.update_one(
    ...
    await request.app.ctx.mongo.reviews.insert_one(document=doc)
...
async def movie_reviews_list_by_date(request: Request):
    ...
    cursor = request.app.ctx.mongo.reviews.find({'created_at': {'$gt': delta}}) \
...
async def movie_reviews_list_by_rating(request: Request):
    ...
    top_reviews_cursor = request.app.ctx.mongo.reviewMarks.aggregate(pipeline)
    obj_to_rating = {ObjectId(x['_id']): x['rating'] for x in await top_reviews_cursor.to_list(None)}

    cursor = request.app.ctx.mongo.reviews.find({'_id': {'$in': [*obj_to_rating.keys()]}}) \
        .skip(page * per_page) \
        .limit(per_page)
...
async def review_mark(request: Request, movie_id: str, user_id: str, review_id: str, mark: int):
    doc = await request.app.ctx.mongo.reviewMarks.find_one({'_id': ObjectId(review_id)})
    if doc is not None:
        return text('Bad request', 403)

    await request.app.ctx.mongo.reviewMarks.insert_one({'user_id': user_id,
                                                        'movie_id': movie_id,
